...
class TestPasswordRecovery:
    ...
    @pytest.mark.skip
    def test_password_recovery_with_registered_email(self, user):
        email = user.email
        email_password = user.email_password
        old_password = user.password
        new_password = 'brand_n3w_test_p4ssword'
    
        self.start_password_recovery(user.base_url, email)
        sleep(5)
        token = self.get_password_reset_token(email, email_password)
        self.change_password_with_token(user.cluster, token, new_password)
        self.auth(user.cluster, email, new_password)
        self.start_password_recovery(user.base_url, email)
        sleep(5)
        token = self.get_password_reset_token(email, email_password)
        self.change_password_with_token(user.cluster, token, old_password)
        self.auth(user.cluster, email, old_password)
    ...
    def test_password_recovery_with_invalid_email_returns_error(self, user, invalid_email):
        """
        Check password recovery with invalid emails
        """
        with pytest.raises(requests.HTTPError) as e:
            self.start_password_recovery(user.base_url, invalid_email)
    ...
    def start_password_recovery(self, base_url, email):
        body = {'email': email}
        response = requests.post(base_url + endpoints.RECOVERY_START, json=body)
        response.raise_for_status()
    
    def get_password_reset_token(self, email, email_password):
    ...
    def change_password_with_token(self, cluster, token, new_password):
