...
def test_sign_in_with_valid_credentials_have_correct_user_information(user):
    ...
    assert actual_user_values['email'] == expected_results.TEST_USER_INFORMATION['email']
    assert actual_user_values['name'] == expected_results.TEST_USER_INFORMATION['name']
    assert actual_user_values['is_active'] == expected_results.TEST_USER_INFORMATION['is_active']
    assert actual_user_values['account']['id'] == expected_results.TEST_USER_INFORMATION['account']['id']
    assert actual_user_values['account']['address'] == expected_results.TEST_USER_INFORMATION['account']['address']
...
def test_sign_in_with_non_existant_email_returns_error():
    ...
    assert e.value.response.json() == expected_results.SIGN_IN_WITH_NON_EXISTENT_EMAIL_ERROR
